...
function configure_para_mt6877() {
	MemTotalStr=`cat /proc/meminfo | grep MemTotal`
	MemTotal=${MemTotalStr:16:8}
	product_id=`getprop ro.boot.prjname`
        para_path=/sys/module/oplus_bsp_zram_opt/parameters

	#for Roma  V align memory parameter with U
	case $product_id in
	"22612"|"22693"|"226B1"|"22694")
		if [ $MemTotal -le 6291456 ]; then
                        echo 62112 > $para_path/oplus_extra_free_kbytes
			echo 18878 > /proc/sys/vm/min_free_kbytes
			echo 10 > /proc/sys/vm/watermark_scale_factor
		elif [ $MemTotal -le 8388608 ]; then
                        echo 63000 > $para_path/oplus_extra_free_kbytes
			echo 22136 > /proc/sys/vm/min_free_kbytes
			echo 25 > /proc/sys/vm/watermark_scale_factor
		else
                        echo 62200 > $para_path/oplus_extra_free_kbytes
			echo 27446 > /proc/sys/vm/min_free_kbytes
                        echo 25 > /proc/sys/vm/watermark_scale_factor
		fi
	;;
	#for Chengdu  V align memory parameter with U
	"22629"|"22710"|"22711"|"22633"|"22712"|"22713")
		if [ $MemTotal -le 6291456 ]; then
                        echo 62112 > $para_path/oplus_extra_free_kbytes
			echo 18924 > /proc/sys/vm/min_free_kbytes
			echo 10 > /proc/sys/vm/watermark_scale_factor
		elif [ $MemTotal -le 8388608 ]; then
                        echo 63000 > $para_path/oplus_extra_free_kbytes
			echo 22136 > /proc/sys/vm/min_free_kbytes
			echo 25 > /proc/sys/vm/watermark_scale_factor
                        ...
                        echo 62200 > $para_path/oplus_extra_free_kbytes
			echo 27456 > /proc/sys/vm/min_free_kbytes
                        echo 25 > /proc/sys/vm/watermark_scale_factor
		fi
	;;
	#for Tacoo  V align memory parameter with U
	"23686"|"23687"|"23689"|"23707"|"23709")
		if [ $MemTotal -le 6291456 ]; then
                        echo 39461 > $para_path/oplus_extra_free_kbytes
			echo 18878 > /proc/sys/vm/min_free_kbytes
			echo 10 > /proc/sys/vm/watermark_scale_factor
		elif [ $MemTotal -le 8388608 ]; then
                        echo 64500 > $para_path/oplus_extra_free_kbytes
			echo 22106 > /proc/sys/vm/min_free_kbytes
			echo 25 > /proc/sys/vm/watermark_scale_factor
		else
                        echo 73728 > $para_path/oplus_extra_free_kbytes
			echo 22106 > /proc/sys/vm/min_free_kbytes
                        echo 25 > /proc/sys/vm/watermark_scale_factor
		fi
	;;
	#for Burberry AvatarH chanel align memory parameter with U
	"23035"|"23321"|"22277"|"23231")
		if [ $MemTotal -le 8388608 ]; then
			echo 12374 > /proc/sys/vm/min_free_kbytes
			echo 25 > /proc/sys/vm/watermark_scale_factor
		else
			echo 13728 > /proc/sys/vm/min_free_kbytes
			echo 25 > /proc/sys/vm/watermark_scale_factor
		fi
	;;
	*)
		echo -e "product_id is not belong Roma/Chengdu/Tacoo-V"
	;;
	esac
}
...
function oplus_configure_hybridswap() {
	...
	;;
	*)
		echo -e "product_id is not belong Roma/Cchengdu/Tacoo-V"
	;;
	esac
